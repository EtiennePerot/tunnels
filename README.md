...
Aliases are global across all configuration files. You usually do not have to write `ports` sections yourself; the default set provided at `conf.d.sample/autogenerated-ports.yml` and the aliases provided at `conf.d.sample/common-port-aliases.yml` should cover most use-cases.
...
* `privateAddresses`: An IP subnet to reserve for `tunnels` use. This subnet will be locally routed when starting `tunnels`. The default is `10.42.0.0/16`, but you may want to use something like `127.42.0.0/16` for peace of mind. Do **not** use `127.0.0.1/8`, as that may break some other applications. Larger subnets mean being able to hold more IP-domain associations at once.
